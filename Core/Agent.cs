...
public class Agent(string name, GoapPlanner planner)
{
    private readonly GoapPlanner _planner = planner;
    ...
    /// </summary>
    public Dictionary<string, bool> WorldState { get; } = [];
    ...
    public void RegisterSensor<T>(string key, ISensor<T> sensor) where T : class => _sensors[key] = (ISensor<object>)(object)sensor;

    /// <summary>
    /// Registers a checklist condition with the agent.
    /// </summary>
    /// <param name="condition">The condition to register.</param>
    public void RegisterCondition(IChecklistCondition condition) => _checklistConditions.Add(condition);
